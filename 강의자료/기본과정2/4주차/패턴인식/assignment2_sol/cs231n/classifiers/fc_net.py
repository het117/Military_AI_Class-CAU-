...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
                 ...
                 weight_scale=1e-2, dtype=np.float32,dropout_keep_ratio=1 ,seed=None):
        """
        ...
        ############################################################################
        #레이어를 그냥 계속 연결되게 하기위해 연달아서 합성
        integration = np.hstack([input_dim, hidden_dims, num_classes])
        
        #dropout setting 
        dropout=dropout_keep_ratio

        #정규 분포 형태로 초기화 진행
        for i in range(self.num_layers):
            self.params['W'+str(i+1)] = weight_scale*np.random.randn(integration[i],integration[i+1])
            self.params['b'+str(i+1)] = np.zeros(integration[i+1])
        
        #batchnormalization 파라미터도 dictionary 형태로 제작
        if self.normalization != None:
            for i in range(self.num_layers-1):
                self.params['gamma'+str(i+1)] = np.ones(integration[i+1])
                self.params['beta' +str(i+1)] = np.zeros(integration[i+1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        #gradient 게산을 위해 역으로 for loop 진행
